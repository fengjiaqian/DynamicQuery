...
define(['app', 'directives/biz-Select',
    ...
    app.controller('InStockConfigUpdateController', ['$http', '$scope', '$stateParams','findDictionaryByCodeService', 'productDisplayCategoryService', 'InStockConfigService','EarningReportService','pagedataLoading','WarehouseInventoryService',
        function ($http, $scope,$stateParams, findDictionaryByCodeService, productDisplayCategoryService, InStockConfigService, EarningReportService, pagedataLoading,WarehouseInventoryService) {
            $scope.vo = {};
            $scope.vo.status = 0;
            //仓库集合
           ...
           $scope.updateSelection = function (item, type) {
		    ...
		    $scope.selectOne = function (id) {
				...
				$scope.vo.twoCategoryList.forEach(function(v) {
        			$scope.levelTwoArr.forEach(function(x){
        				if (x.id == v.twocategoryId) {
        					x.flag = true;
        				}
        			});
        		});
            ...
            //初始化修改数据
            var initInStockConfig = function() {
            	InStockConfigService.getInStockConfigById($stateParams.configId).then(function (response) {
                    if (response.success == true) {
                    	setInStockConfigData(response.data);
                    } else {
                        bootbox.alert(response.error);
                    }
                })
            ...
            //把数据设置到页面控件
            var setInStockConfigData = function(rvo) {
            	$scope.vo.id = rvo.id;
            	//设置策略名称
            	$scope.vo.configName = rvo.configName;
            	$scope.vo.status = rvo.status;
            	//设置仓库集合
            	if (rvo.warehouseList && rvo.warehouseList.length > 0) {
            		$scope.vo.warehouseList = rvo.warehouseList;
            		$scope.vo.warehouseList.forEach(function(v) {
            			$scope.warehouseArr.push({id:v.warehouseId, name:v.warehouseName, flag:true});
            		});
            	}
                
                //设置一级类目集合
            	if (rvo.oneCategoryList && rvo.oneCategoryList.length > 0) {
            		$scope.vo.oneCategoryList = rvo.oneCategoryList;
            		$scope.vo.oneCategoryList.forEach(function(v) {
            			$scope.levelOneArr.forEach(function(x){
            				if (x.id == v.onecategoryId) {
            					x.flag = true;
            				}
            			});
            		});
            	...
            	//设置二级类目集合
            	if (rvo.twoCategoryList && rvo.twoCategoryList.length > 0) {
            		$scope.vo.twoCategoryList = rvo.twoCategoryList;
            		$scope.vo.twoCategoryList.forEach(function(v) {
            			$scope.levelTwoArr.forEach(function(x){
            				if (x.id == v.twocategoryId) {
            					x.flag = true;
            	...
            	//设置特殊产品集合
            	if (rvo.productList && rvo.productList.length > 0) {
            		$scope.vo.productList = rvo.productList;
            		$scope.vo.productList.forEach(function(v) {
            			$scope.levelSpecialArr.push({
            				cityName:v.categoryname,
            				productBrand:v.brandname,
            				productSkuId:v.productskuspecId, 
                ...
                //设置入库提醒条件
                $scope.vo.remindCondition = rvo.remindCondition;
                if (rvo.remindCondition.indexOf("/") == -1) {
                	$scope.vo.remindType2 = rvo.remindCondition;
                } else {
                	$scope.vo.remindType1 = rvo.remindCondition;
                }
                //设置禁止入库条件
                $scope.vo.banCondition = rvo.banCondition;
                if (rvo.banCondition.indexOf("/") == -1) {
                	$scope.vo.banType2 = rvo.banCondition;
                } else {
                	$scope.vo.banType1 = rvo.banCondition;
                }
                //设置强制校验逻辑
                if (rvo.constraintCondition) {
                	$scope.vo.constraintCondition = rvo.constraintCondition;
                	$scope.constraintArr.forEach(function(v){
                		if (rvo.constraintCondition.indexOf(v.id) != -1) {
                			v.flag = true;
                		}
                	});
                }
